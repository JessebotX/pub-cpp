...
{
	bool_options.emplace_back(flag_names_true, flag_names_false, value_default);
...
{
	string_options.emplace_back(flag_names, value_default);
...
{
	int_options.emplace_back(flag_names, value_default);
...
{
	float_options.emplace_back(flag_names, value_default);
...
App::parse(std::span<char*> program_args, std::size_t start_index)
{
	for (std::size_t i = start_index; i < program_args.size(); i++) {
		...
		auto is_flag_equal = [&](auto flag) -> bool {
			return std::strncmp(curr, flag, std::strlen(flag)) == 0;
		};
		...
		// string options
		{
			...
			if (it != string_options.end()) {
				if (i + 1 >= program_args.size()) {
					return std::unexpected{App::ErrParseMissingValue{
						.flag = curr
					}};
				}

				it->set_value(StringValue{program_args[i + 1]});
				i++;
				continue;
			}
		}
		...
		// int options
		{
			auto it = std::find_if(int_options.begin(), int_options.end(), [&](auto& opt) {
				auto it = std::find_if(opt.flag_names.begin(), opt.flag_names.end(), is_flag_equal);
				return it != opt.flag_names.end();
			});
			if (it != int_options.end()) {
				...
				try {
					it->set_value(IntValue{std::stoll(program_args[i + 1])});
				} catch (std::exception&) {
					return std::unexpected{App::ErrParseInvalidOptValue<IntValue>{
						.flag = curr,
		...
		// float options
		{
			auto it = std::find_if(float_options.begin(), float_options.end(), [&](auto& opt) {
				auto it = std::find_if(opt.flag_names.begin(), opt.flag_names.end(), is_flag_equal);
				return it != opt.flag_names.end();
			});
			if (it != float_options.end()) {
				if (i + 1 >= program_args.size()) {
					return std::unexpected{App::ErrParseMissingValue{
						.flag = curr
					}};
				...
				try {
					it->set_value(FloatValue{std::stold(program_args[i + 1])});
				} catch (std::exception&) {
					return std::unexpected{App::ErrParseInvalidOptValue<FloatValue>{
						.flag = curr,
						.value = program_args[i + 1]
					}};
				}
